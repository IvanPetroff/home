inherited FraSZ: TFraSZ
  inherited DS: TOraQuery
    SQL.Strings = (
        ...
        'select kod, naim, r_sort, unizak, pzak, izd, sum(kol) kol, sum(k' +
        'ol*cena) kol_cena from ('
      ...
      '      ,extractvalue(column_value,'#39'/ROW/CENA'#39') cena'
      
        '--from table(xmlsequence( class_spreadlist(:1).getSpreadList(xml' +
        'type(:x)).extract('#39'/ROWSET/ROW'#39')))'
      
        'from table(xmlsequence( c_spreadlist.create_obj(:1).getSpreadLis' +
        't(xmltype(:x)).extract('#39'/ROWSET/ROW'#39')))'
      ')'
      'group by kod, naim, r_sort, unizak, pzak, izd')
    ParamData = <
      item
        DataType = ftUnknown
        Name = '1'
